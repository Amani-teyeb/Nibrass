...
export const register = (user, history) => async (dispatch) => {
  ...
  try {
    let result = await axios.post("/api/teacher/register", user);
    //succees action
...
export const login = (user, history) => async (dispatch) => {
  dispatch({ type: LOAD_USER });
  try {
    let result = await axios.post("/api/teacher/login", user);
...
export const current = () => async (dispatch) => {
  try {
    ...
    let result = await axios.get("/api/teacher/current", config);
